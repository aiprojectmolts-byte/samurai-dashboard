...
export async function POST() {
  try {
    ...
    const items: any[] = []
    // 最新セッションIDのエントリのみ同期
    const latestSessionId = expressions[0]?.sessionId || ''
    const targetSession = latestSessionId
      ? expressions.filter((e: any) => e.sessionId === latestSessionId)
      : expressions.slice(0, 3)
