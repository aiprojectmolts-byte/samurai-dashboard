    ...
    <div style={overlay}>
      <div style={modal}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
          ...
          <label style={lbl}>
            ...
            <select value={form.施策} onChange={e => set('施策', e.target.value)} style={inp}>
              {['施策1','施策2','施策3','施策4','施策5','施策6','施策7'].map(s => (
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
            <label style={lbl}>
              開始日
              <input type="date" value={form.s} onChange={e => set('s', e.target.value)} style={inp} />
            </label>
            <label style={lbl}>
              終了日
              <input type="date" value={form.e} onChange={e => set('e', e.target.value)} style={inp} />
            </label>
          </div>

          <label style={lbl}>
            担当
            <select value={form.own} onChange={e => set('own', e.target.value as Task['own'])} style={inp}>
        ...
        <div style={{ display: 'flex', gap: 8, marginTop: 20, justifyContent: 'flex-end' }}>
          {!isNew && (
            <button onClick={() => { onDelete(form); onClose() }} style={delBtn}>削除</button>
          )}
          <button onClick={onClose} style={cancelBtn}>キャンセル</button>
          <button
            onClick={() => { if (form.name.trim()) { onSave(form, isNew); onClose() } }}
            style={saveBtn}
          >保存</button>
        </div>
      </div>
...
const modal: CSSProperties = { background: 'var(--paper)', border: '0.5px solid var(--b1)', borderRadius: 'var(--r)', padding: 24, width: 400, fontFamily: 'inherit', boxShadow: '0 8px 32px rgba(0,0,0,0.12)' }
